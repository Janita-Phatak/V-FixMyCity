    ...
    <div id="blog" class="bg-gray-50 px-4 pb-5">
        <div class="mx-auto container">
            <div v-if="users.length > 0"
                class="w-full max-w-md p-4 bg-white border rounded-lg shadow-md sm:p-8 mt-4 lg:mt-6 mx-auto">
                <div class="flow-root">
...
export default {
    ...
    methods: {
        async loadData() {
            this.isLoading = true;
            await axios.get(properties.server + "/user/users/filter/userType?userType=worker").then(
                (res) => {
                    this.users = res.data;
                }
            )
